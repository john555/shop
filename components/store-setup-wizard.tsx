...
import { Sun, Moon, ArrowRight, ArrowLeft, ShoppingBag, Home, Car } from 'lucide-react'
...
const StoreSetupContext = React.createContext<StoreSetupContextType | undefined>(undefined)
...
const StoreSetupProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateStoreData = React.useCallback((data: Partial<typeof storeData>) => {
    setStoreData((prev) => {
...
const StepWrapper: React.FC<{
  ...
  const { currentStep, visibleSteps } = useStoreSetup()
...
const StoreNameStep: React.FC<{ stepIndex: number }> = ({ stepIndex }) => {
  const { storeData, updateStoreData, setCurrentStep, setVisibleSteps, currentStep, visibleSteps } = useStoreSetup()
    ...
    <StepWrapper icon="🏪" title="Store Name" stepIndex={stepIndex}>
      <div className="space-y-4">
        ...
        <p className="text-muted-foreground">
          <TypeAnimation
            sequence={[1000, "Let's start by giving your store a name. This will be the first thing your customers see.", 1000, () => setAnimationComplete(true)]}
            wrapper="span"
            speed={50}
            style={{ display: 'inline-block' }}
            cursor={false}
        ...
        >
          ...
          <div className="flex justify-end mt-4">
            <Button
              ...
              className={`transition-all duration-300 ${currentStep !== stepIndex ? "invisible" : "visible"}`}
            >
...
const StoreHandleStep: React.FC<{ stepIndex: number }> = ({ stepIndex }) => {
  const { storeData, updateStoreData, setCurrentStep, setVisibleSteps, currentStep, visibleSteps } = useStoreSetup()
  const [storeHandle, setStoreHandle] = React.useState(storeData.handle || storeData.name.toLowerCase().replace(/\s+/g, '-'))
  const [animationComplete, setAnimationComplete] = React.useState(false)
  ...
  React.useEffect(() => {
    if (storeData.handle !== storeHandle) {
      updateStoreData({ handle: storeHandle })
    }
  }, [storeHandle, updateStoreData, storeData.handle])
  ...
  return (
    <StepWrapper icon="🔗" title="Store Handle" stepIndex={stepIndex}>
      <div className="space-y-4">
        ...
        <p className="text-muted-foreground">
          <TypeAnimation
            sequence={[1000, "Your store handle will be used in your store's URL.", 1000, () => setAnimationComplete(true)]}
            wrapper="span"
            speed={50}
            style={{ display: 'inline-block' }}
            cursor={false}
        ...
        >
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Your store will be available at: https://yourplatform.com/{storeHandle}
            </p>
          </div>
...
const StoreTypeStep: React.FC<{ stepIndex: number }> = ({ stepIndex }) => {
  ...
  const storeTypes = [
    { id: 'physical', label: 'Physical Goods', icon: ShoppingBag, disabled: false },
    { id: 'real-estate', label: 'Real Estate', icon: Home, disabled: true },
  ...
  return (
    <StepWrapper icon="📦" title="Store Type" stepIndex={stepIndex}>
      <div className="space-y-4">
        ...
        <p className="text-muted-foreground">
          <TypeAnimation
            sequence={[1000, "Select the primary category of items you'll be selling. This helps us tailor your store experience.", 1000, () => setAnimationComplete(true)]}
            wrapper="span"
            speed={50}
            style={{ display: 'inline-block' }}
            cursor={false}
...
const StoreCurrencyStep: React.FC<{ stepIndex: number }> = ({ stepIndex }) => {
  ...
  const handleFinish =    () => {
    console.log('Wizard completed:', storeData)
  }
  ...
  return (
    <StepWrapper icon="💰" title="Store Currency" stepIndex={stepIndex}>
      <div className="space-y-4">
        ...
        <p className="text-muted-foreground">
          <TypeAnimation
            sequence={[1000, "Your store will use Ugandan Shilling (UGX) for all transactions.", 1000, () => setAnimationComplete(true)]}
            wrapper="span"
            speed={50}
            style={{ display: 'inline-block' }}
            cursor={false}
        ...
        >
          <div className="space-y-2">
            ...
            <Select onValueChange={handleCurrencyChange} value="UGX" disabled>
              <SelectTrigger className="w-full">
                <SelectValue>Ugandan Shilling (UGX)</SelectValue>
              </SelectTrigger>
              <SelectContent>
